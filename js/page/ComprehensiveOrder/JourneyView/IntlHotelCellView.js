...
class IntlHotelCellView extends React.Component {
    render() {
        ...
        var ArrivalDate = item.ArrivalTime.slice(0, 10);
        var DepartureDate = item.DepartureTime.slice(0, 10);
        var week = Util.Date.toDate(item.DepartureTime).getWeek();
        let seat = item.Seat && item.Seat.length > 22 ? item.Seat.substr(0, 22) + '...' : item.Seat;
        return (
                ...
                <View style={{ backgroundColor: Theme.greenBg, borderRadius: 4, paddingHorizontal: 10 }}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                        <View style={{ alignItems: 'flex-start', padding: 10 }}>
                            <CustomText text={DepartureDate} style={{ fontSize: 16, fontWeight: '300' }} />
                        ...
                        <View style={{ alignItems: 'flex-end', padding: 10 }}>
                            <CustomText text={ArrivalDate} style={{ fontSize: 16, fontWeight: '300' }} />
                            <CustomText text={item.ArrivalLocation} style={{ fontSize: 12 }} />
                        </View>
                    </View>
                    <View>
                        <CustomText text={seat} style={{ marginLeft: 8, fontSize: 11, color: Theme.assistFontColor }} />
                    </View>
...
export default function(props) {
    ...
    return <IntlHotelCellView {...props} navigation={navigation} />;
}
