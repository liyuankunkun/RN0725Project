...
class HotelView extends React.Component {
    render() {
        ...
        let travellerNames = []
        item.Travellers?.map((travel_item)=>{
            ...
            <View style={styles.borderStyle}>
                ...
                <View style={styles.titleStyle}>
                   <View style={{flexDirection:'row',justifyContent:'space-between',flexWrap:'wrap'}}>
                      <CustomText text={Util.Parse.isChinese()? hotel.Name: hotel.EnName } />
                ...
                <View style={{flexDirection:'row', paddingHorizontal:20, paddingVertical:3 ,justifyContent:'space-between',alignItems:'center'}}>
                   <View style={{flexDirection:'row'}}>
                      ...
                      <CustomText text={Util.Parse.isChinese()? hotel.Address: hotel.EnAddress} style={{marginLeft:5,color:Theme.assistFontColor}} />
                   </View> 
                   <View style={{flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}>
                      {
                        item.InternalOrder.Status === 3  && item.InternalOrder.Guarantee.NeedGuaranteeValidation?
                            <TouchableOpacity style={{ paddingVertical:3,paddingHorizontal:6,backgroundColor:Theme.theme,borderRadius:5}}
                                            onPress={this.props.guaranteedCallback}
                            >
                                <CustomText text={'提交验证码'} style={{color:'#fff',fontSize:12}} />
                            </TouchableOpacity>
                        :null
                      }
                   </View>
                   <View style={{flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}>
                      {
                        item.InternalOrder.Status==2 &&item.InternalOrder.RatePlan.IsNeedCreditCard && item.InternalOrder?.RatePlan?.PrepayRules?.[0]?.NeedCvv?
                            <TouchableOpacity style={{ paddingVertical:3,paddingHorizontal:6,backgroundColor:Theme.theme,borderRadius:5}}
                                            onPress={this.props.needCvvCallback}
                            >
                ...
                <TouchableOpacity style={styles.personStyle} onPress={this.props.callback} >
                        <View style={{flexDirection:'row'}}>
                            ...
                            <CustomText text={travellerString } numberOfLines={1} style={{fontSize: 14}}/>
                        </View>
                   <View style={{flexDirection:'row', alignItems:'center',justifyContent:'space-between'}}>
...
export default function(props) {
    ...
    return (
        <HotelView {...props} navigation={navigation} />
    )
}
