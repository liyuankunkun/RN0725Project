...
export default class HotelOrderListScreen extends SuperView {
    constructor(props) {
        super(props);
    ...
    _pay = (obj) => {
        if(obj.Status==1){
            this.push('HotelGuarantee', { OrderId: obj.Id });
        }else{
            this.showLoadingView();
            HotelService.HotelOrderPayment({ OrderId: obj.Id }).then(response => {
                this.hideLoadingView();
                if (response && response.success) {
                    this.push('HotelPayment', { SerialNumber: response.data.SerialNumber,Id:obj.Id,from:'hotel' });
                } else {
     ...
     */
    _refundPay= (obj) => {
        HotelService.HotelOrderRefundPayInfo({ BusinessId: obj.Id }).then(response => {
            this.hideLoadingView();
            if (response && response.success) {
                this._toPay({ SerialNumber: response.data.SerialNumber,Id:obj.Id });
            } else {
                this.toastMsg(response.message || '获取支付信息失败');
            }
        }).catch(error => {
    ...
    //         } else {
    //             this.toastMsg(error.message || '催审订单失败');
    //         }
    //     }).catch(error => {
    //         this.hideLoadingView();
    ...
    _guaranteeValidationBtn = (item) => {
        this.push('HotelGuranteeMessageVertify',{OrderId:item.Id,CreditCard : null,});
    }
     /**
     *  填写CVV
     */
    _creditCVVBtn = (item) => {
        this.push('CvvScreen',{OrderId:item.Id});
    }

    renderBody() {
        const { dataList, isLoading, isLoadingMore, isNoMoreData, keyword,PaymentInfo,refundOrder,options } = this.state;
        return (
            <View style={{ flex: 1 }}>
