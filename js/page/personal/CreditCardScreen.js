...
class CreditCardScreen extends SuperView {
    constructor(props) {
        super(props);
    ...
    _rightHeaderView = () => {
        return (
          <TouchableHighlight underlayColor='transparent' onPress={this._toSettting}>
            <AntDesign name={'pluscircleo'} color={Theme.theme} size={20} style={{ paddingRight: 16 }} />
          </TouchableHighlight>
        )
    }
    ...
    _loadCardList = () => {
        const { OtherBookingId } = this.params;
        CommonService.GetCreditCardList().then(response => {
    ...
    _loadOtherCardList(){
        let model = {
            EmployeeId:this.params.OtherBookingId
        }
        CommonService.HandShakeGetCreditCardList(model).then(response => {
            if (response && response.success ) {
                if (response.data) {
                    response.data.map((item)=>{
                        item.EmployeeId = this.params.OtherBookingId
                    })
                    this.state.ListData = this.state.ListData.concat(response.data);
                    this.setState({
                        isLoading: false,
                        loadingMore: false,
                    })
                    ...
                    if(!response.data){
                        this.setState({
                            showErrorMessage: '维护的信用卡类型与酒店担保支持信用卡类型不符'
                        })
                    }
            } else {
                // this.toastMsg('维护的信用卡类型与酒店担保支持信用卡类型不符');
                this.setState({
                    showErrorMessage: response.message || '维护的信用卡类型与酒店担保支持信用卡类型不符'
                })
            }
        }).catch(error => {
            this._detailError();
    ...
    _deleteClick = (obj)=>{
        ...
        CommonService.DeleteCreditCard(model).then(response => {
        ...
        return (
            <View style={{ flex: 1 }}>
               ...
               !ListData && showErrorMessage?
                ...
                {ViewUtil.getThemeButton('添加信用卡',this._toSettting)}
            </View>
        )
    }
}
...
export default connect(getStateProps)(CreditCardScreen);
